...
export class ProductsController {
  ...
  private mapProductResponse(p: any) {
    const precioNum =
      typeof p.precio === "string" ? parseFloat(p.precio as any) : p.precio;
    return {
      ...p,
      id: p.id_producto,
      name: p.nombre,
  ...
  async create(@Body() createProductDto: CreateProductDto) {
    ...
    if (createProductDto.localId !== undefined) {
      await this.productsService.setStockLocal(
        product.id_producto,
        createProductDto.localId,
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: "ID del local para obtener stock específico",
  })
  @ApiQuery({
    name: "search",
    required: false,
  ...
  findAll(
    ...
    return this.productsService
      .findAll(active)
      .then((products) => products.map((p) => this.mapProductResponse(p)));
  }
  ...
  findOne(@Param("id", ParseIntPipe) id: number) {
    return this.productsService
      .findOne(id)
      .then((p) => this.mapProductResponse(p));
  }
  ...
  async update(@Param("id", ParseIntPipe) id: number, @Body() body: any) {
    ...
    if (hasLocalId) {
      ...
      if (!Number.isFinite(localId)) {
        throw new BadRequestException(
          "El ID del local debe ser un número válido",
        );
      }
  ...
  @ApiOperation({
    summary: "Obtener stock de un producto en un local específico",
  })
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "Actualizar stock de un producto en un local específico",
  })
  @ApiParam({ name: "productId", description: "ID del producto" })
  @ApiParam({ name: "localId", description: "ID del local" })
  ...
  updateStockLocal(
    ...
    return this.productsService.setStockLocal(
      productId,
      localId,
      body.stock,
      body.precio_local,
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "Reducir stock de un producto en un local específico",
  })
  @ApiParam({ name: "productId", description: "ID del producto" })
  @ApiParam({ name: "localId", description: "ID del local" })
  ...
  reduceStockLocal(
    ...
    return this.productsService.updateStockLocal(
      productId,
      localId,
      -body.quantity,
    );
