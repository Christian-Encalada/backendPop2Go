...
export class AddressesService {
   ...
   */
  async create(
    addressData: CreateAddressWithUserDto,
    ...
    if (
      ...
      throw new ForbiddenException(
        "No tienes permisos para crear una dirección para otro usuario",
      );
    }
    ...
    if (addressData.id_local) {
      ...
      if (!store) {
        throw new BadRequestException(
          "El local especificado no existe o no está activo",
        );
      }
    }
  ...
  async findOne(
    ...
    if (
      ...
      throw new ForbiddenException(
        "No tienes permisos para ver direcciones de otras ciudades",
      );
    }
  ...
  async updateStore(
    ...
    try {
      ...
      const store = await this.addressRepository.manager
        .getRepository("Store")
        .findOne({
          where: { id_local },
          relations: ["city"],
